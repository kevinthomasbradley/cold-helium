...
uint256 hashGenesisBlock("0x69668a1482b7a42ad1777b41f7e17e620f9decc4bed931e261bf18be2af00e7c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Heliumcoin: starting difficulty is 1 / 2^12
...
{
    // Heliumcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Heliumcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Heliumcoin: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // Heliumcoin: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Heliumcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Heliumcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x69668a1482b7a42ad1777b41f7e17e620f9decc4bed931e261bf18be2af00e7c");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Today, I made quite a bit on the cryptocurrencies BTC, LTC, ETH and XRP";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("280984710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1513090772;
        ...
        assert(block.hashMerkleRoot == uint256("0x9cbb1eff18076b35741ea5ef8a98ebb9f4fb08820e569f31b9e8cfa21821aaea"));

	// Code to create new MerkleRoot if not exists
	if (true && block.GetHash() != hashGenesisBlock)
	{
		printf("Searching for genesis block...\n");
		// This will figure out a valid has and Nonce if you're
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Heliumcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("HeliumcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("HeliumcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static HeliumcoinMiner(CWallet *pwallet)
{
    printf("HeliumcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("heliumcoin-miner");
    ...
    try { loop {
        ...
        printf("Running HeliumcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("HeliumcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&HeliumcoinMiner, pwallet));
}
